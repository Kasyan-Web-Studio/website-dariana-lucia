...
export function PageChrome({ children }: { children: React.ReactNode }) {
  ...
  return <div className="page-shell"><header className="site-header page-header"><Link className="header-logo" href="/" aria-label="Dariana și Lucia — Acasă"><img src="/assets/brand/logo-dl-seal-v1.png?v=4" alt="Dariana & Lucia — Nails & Lashes" /></Link><button ref={menuButtonRef} className={`menu-toggle${menuOpen ? ' is-open' : ''}`} type="button" aria-label={menuOpen ? 'Închide meniul' : 'Deschide meniul'} aria-expanded={menuOpen} aria-controls="page-menu" onClick={() => setMenuOpen((open) => !open)}><span /><span /><span /></button></header>{children}<aside ref={menuPanelRef} id="page-menu" className={`menu-overlay${menuOpen ? ' is-open' : ''}`} aria-hidden={!menuOpen} aria-modal="true" aria-label="Meniu principal"><div className="menu-backdrop" aria-hidden="true" onClick={closeMenu} /><div className="menu-inner"><img className="brand-logo brand-logo-mark menu-logo" src="/assets/brand/logo-dl-seal-v1.png?v=4" alt="Dariana & Lucia" /><p className="menu-kicker">The details make the difference <span>✦</span></p><nav><ol>{homeContent.menu.map((item, index) => <li key={item.label}><span className="menu-index">0{index + 1}</span><a href={item.href} aria-current={item.href === pathname || (item.href === '/' && pathname === '/') ? 'page' : undefined} onClick={closeMenu}>{item.label}</a></li>)}</ol></nav><div className="menu-footer"><span>Dariana &amp; Lucia Studio</span><span>Est. 2026</span></div></div></aside><footer className="site-footer"><img className="brand-logo brand-logo-no-slogan" src="/assets/brand/logo-dl-seal-v1.png?v=4" alt="Dariana & Lucia — Lashes & Nails Studio" /><p>Beauty, shaped in every detail.</p><ContactSummary /></footer></div>;
}

export function PageCta() { return <section className="page-cta" aria-labelledby="page-cta-title"><h2 id="page-cta-title">Alege serviciul potrivit pentru tine</h2><p>Continuă către programare și selectează serviciul dorit.</p><div className="pre-footer-actions"><a className="button button-primary" href="/programare?categorie=gene">Programare gene</a><a className="button button-primary" href="/programare?categorie=unghii">Programare unghii</a></div></section>; }
