...
export function PageChrome({ children }: { children: React.ReactNode }) {
  ...
  return <div className="page-shell"><header className="site-header page-header"><Link className="header-logo" href="/" aria-label="Dariana și Lucia — Acasă"><img src="/assets/brand/logo-dl-round-v1.png?v=3" alt="Dariana & Lucia — Nails & Lashes" /></Link><button ref={menuButtonRef} className={`menu-toggle${menuOpen ? ' is-open' : ''}`} type="button" aria-label={menuOpen ? 'Închide meniul' : 'Deschide meniul'} aria-expanded={menuOpen} aria-controls="page-menu" onClick={() => setMenuOpen((open) => !open)}><span /><span /><span /></button></header>{children}<aside ref={menuPanelRef} id="page-menu" className={`menu-overlay${menuOpen ? ' is-open' : ''}`} aria-hidden={!menuOpen} aria-modal="true" aria-label="Meniu principal"><div className="menu-backdrop" aria-hidden="true" onClick={closeMenu} /><div className="menu-inner"><img className="brand-logo brand-logo-mark menu-logo" src="/assets/brand/logo-dl-round-v1.png?v=3" alt="Dariana & Lucia" /><p className="menu-kicker">The details make the difference <span>✦</span></p><nav><ol>{homeContent.menu.map((item, index) => <li key={item.label}><span className="menu-index">0{index + 1}</span><a href={item.href} aria-current={item.href === pathname || (item.href === '/' && pathname === '/') ? 'page' : undefined} onClick={closeMenu}>{item.label}</a></li>)}</ol></nav><div className="menu-footer"><span>Dariana &amp; Lucia Studio</span><span>Est. 2026</span></div></div></aside><footer className="site-footer"><img className="brand-logo brand-logo-no-slogan" src="/assets/brand/logo-dl-round-v1.png?v=3" alt="Dariana & Lucia — Lashes & Nails Studio" /><p>Beauty, shaped in every detail.</p><ContactSummary /></footer></div>;
}
