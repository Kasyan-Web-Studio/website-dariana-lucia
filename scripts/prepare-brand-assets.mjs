...
await makeImage().extract({ left: 440, top: 20, width: 800, height: 560 }).trim({ background: { r: 0, g: 0, b: 0, alpha: 0 } }).resize({ width: 420, withoutEnlargement: true }).png({ compressionLevel: 9 }).toFile(`${output}/logo-mark-dl.png`);
...
await makeImage().extract({ left: 440, top: 20, width: 800, height: 560 }).trim({ background: { r: 0, g: 0, b: 0, alpha: 0 } }).resize({ width: 64, height: 64, fit: 'contain', background: { r: 0, g: 0, b: 0, alpha: 0 } }).png({ compressionLevel: 9 }).toFile(`${output}/favicon.png`);
